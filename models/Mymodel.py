...
class Mamba_layer(nn.Module):
    ...
    def forward(self, x):
        x = self.mamba_pred(self.mamba(x).transpose(-1,-2)).transpose(-1,-2)
        return x 
...
class TimesBlock(nn.Module):
    def __init__(self, configs):
        ...
        self.conv = nn.Sequential(
            Inception_Block_V1(2, configs.d_ff,
                               num_kernels=configs.num_kernels),
            nn.GELU(),
            Inception_Block_V1(configs.d_ff, 2,
                               num_kernels=configs.num_kernels)
        )
...
class Conv2_block(nn.Module):
    def __init__(self, pred_len, en_c, CH,num_kernels=3,init_weight=True):
        ...
        for i in range(self.num_kernels):
            kernels.append(nn.Conv2d(1, pred_len, kernel_size=3, padding=i))
...
class Model(nn.Module):
    def __init__(self, config, *args, **kwargs):
        ...
        self.layer_norm = nn.LayerNorm((2,config.seq_len,config.d_model))
        self.layer = config.e_layers
        
        self.linear2d_model = nn.Linear(
            6*config.d_model, config.d_model)
    ...
    def forward(self,x_enc, x_mark_enc):
        ...
        for i in range(self.layer):
            enc_out.append(self.layer_norm(self.model[i](inx_list[i])))
    
        out = torch.cat(enc_out, dim=1)
        B, C, P, D = out.shape
        out = out.reshape(B, P, C*D)
        out = self.lanorm_dmodel(self.linear2d_model(out))
        out = self.act(out)
        out = self.lanorm_dmodel(self.mamba_model(out))
